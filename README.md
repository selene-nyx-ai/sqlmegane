...
生成した DELETE / UPDATE は本体の実行前チェックにかけます。問題がなければ何も表示しません。WHERE がない・常に真の WHERE などの危険があれば結果の最上部に「何が問題か・元の SELECT をどう直せば通るか」を出し、「意図どおりです」に確認するまで DELETE / UPDATE と安全実行の枠のコピーを無効にします。注意点（LIKE の先頭 % など）は黄色で出すだけでコピーは止めません。
